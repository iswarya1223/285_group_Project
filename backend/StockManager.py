...
stocks = {
    'Ethical Investing': ["AAPL", "TSLA", "NSRGY"],
    'Growth Investing': ["OXLC", "ECC", "AMD"],
    'Index Investing': ["IXUS", "VTI", "ILTB"],
...
def get_latest_price(strategies):
    ...
    print(base_url + url_filter + '?apikey=96f9ab399c21511a46ae6487adbd7e29')
    response = requests.get(base_url + url_filter + '?apikey=96f9ab399c21511a46ae6487adbd7e29')
    print("response debug tetsing",response.json())
...
def get_weekly_trend(strategies, allocation):
    ...
    for strategy in strategies:
        for ticker in stocks.get(strategy):
            response = requests.get(base_url + ticker + date_filter + '&apikey=96f9ab399c21511a46ae6487adbd7e29')
            if response.status_code != 200:
                Exception("API Error")
            response_json = response.json()
